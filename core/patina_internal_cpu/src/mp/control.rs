...
///
/// The actual dispatch loop is architecture specific, but this consumer provides the
/// common state machine and work handoff.
pub(crate) struct ApConsumer<'a> {
    sm: &'a ApStateMachine,
}

#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub(crate) enum ApAction {
    Idle,
    Executed,
    Exit,
}
...
impl ApConsumer<'_> {
    ...
    pub(crate) fn execute_pending_work(&self) -> ApAction {
    ...
    pub(crate) fn should_wait(&self) -> bool {
        self.sm.load() == RunState::Idle
    }

    /// Address to arm a monitor on, for architectures that have one.
    pub(crate) fn monitor_address(&self) -> *const u8 {
...
mod tests {
    ...
    fn disabled_processor_can_still_finish_its_in_flight_work() {
        ...
        sm.set_enabled(false);
        assert_eq!(ap.execute_pending_work(), ApAction::Executed, "a disabled processor still runs published work");
        assert!(sm.is_finished(id));
        assert_eq!(sm.state(), ProcessorState::Disabled);
